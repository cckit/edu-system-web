...
var Kinect = (function () {
    ...
    function KinectConnector() {
        ...
        var DEFAULT_HOST_URI = "http://10.211.55.3";
        ...
        function KinectSensor(baseEndpointUri, onconnection) {
            ...
            function ajax(method, uri, requestData, success, error) {
                var isPostSent = false;

                var stateSocketManager = new SocketManager(uri+'/'+method, function (socket, message) {
                    if (method === "GET") {
                        var data = JSON.parse(message.data);
                        stateSocketManager.stop();
                        success(data);
                    } else {
                        if (isPostSent) {
                            var data = JSON.parse(message.data);
                        ...
                        switch (event.data.message) {
                            case "imageReady":
                                ...
                                if (metadata.image === undefined) {
                                    ...
                                    metadata.image.onload = function() {
                                        canvasContext.clearRect(0, 0, metadata.canvas.width, metadata.canvas.height);
                                        canvasContext.drawImage(metadata.image, 0, 0, metadata.canvas.width, metadata.canvas.height);

                                        if (metadata.image.src !== undefined) {
                                            URL.revokeObjectURL(metadata.image.src);
                                        }
                                    };
                                }

                                metadata.image.src = event.data.imageUrl;
